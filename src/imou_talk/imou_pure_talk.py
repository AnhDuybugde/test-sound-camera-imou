...
async def send_once(args: argparse.Namespace) -> int:
    supplied_password = args.password or os.environ.get("IMOU_CAMERA_PASSWORD")
    device = None if args.serial and supplied_password else load_device(Path(args.device_json), args.device_index)
    serial = args.serial or device["Sn"]
    password = supplied_password or device["Pwd"]
...
def run_visualtalk(args: argparse.Namespace, password: str, audio: bytes) -> int:
    ...
    for attempt in range(1, args.attempts + 1):
        ...
        try:
            ...
            print(f"Sent {frame_count} DHAV audio frames to the camera.", flush=True)
            return 0
        except (OSError, TimeoutError, EOFError, RuntimeError) as exc:
            last_error = exc
            print(f"Talk attempt {attempt} failed: {exc}", file=sys.stderr, flush=True)
            if attempt < args.attempts:
                time.sleep(args.retry_delay)
        finally:
            if client is not None:
                client.close()
    print(f"Talk was not started; WAV was not sent. Last error: {last_error}", file=sys.stderr, flush=True)
    return 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--password", help="override camera password from device json (or IMOU_CAMERA_PASSWORD)")
